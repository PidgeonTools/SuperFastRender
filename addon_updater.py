...
class Singleton_updater(object):
    ...
    def include_branch_autocheck(self, value):
        try:
            ...
            raise ValueError(
                "include_branch_autocheck must be a boolean value")
    ...
    def get_tags(self):
        ...
        if self.skip_tag != None:
            self._tags = [
                tg for tg in all_tags if self.skip_tag(self, tg) == False]
        else:
            self._tags = all_tags
        ...
        if self._tags == None:
            ...
            if self._verbose:
                ...
                print("{} branch found, no releases".format(
                    branch), self._tags[0])
    ...
    def stage_repository(self, url):
        ...
        try:
            ...
            self.urlretrieve(urllib.request.urlopen(
                request, context=context), self._source_zip)
    ...
    def deepMergeDirectory(self, base, merger, clean=False):
        ...
        if clean == True:
            try:
                ...
                files = [f for f in os.listdir(
                    base) if os.path.isfile(os.path.join(base, f))]
                folders = [f for f in os.listdir(
                    base) if os.path.isdir(os.path.join(base, f))]

                for f in files:
                    os.remove(os.path.join(base, f))
                    print("Clean removing file {}".format(
                        os.path.join(base, f)))
                for f in folders:
                    if os.path.join(base, f) == self._updater_path:
                        continue
                    shutil.rmtree(os.path.join(base, f))
                    print("Clean removing folder and contents {}".format(
                        os.path.join(base, f)))
        ...
        for path, dirs, files in os.walk(base):
            ...
            dirs[:] = [d for d in dirs if os.path.join(
                path, d) not in [self._updater_path]]
            for file in files:
            ...
            for file in files:
                ...
                if os.path.isfile(destFile):
                    ...
                    if replaced:
                        ...
                        if self._verbose:
                            print("Pattern not matched to " +
                                  os.path.basename(destFile)+", not overwritten")
                else:
    ...
    def past_interval_timestamp(self):
        ...
        if "last_check" not in self._json or self._json["last_check"] == "":
            ...
            if delta.total_seconds() > 0:
                ...
                if self._verbose:
                    print("{} Updater: Determined it's not yet time to check for updates".format(
                        self._addon))
                return False
    ...
    def async_check_update(self, now, callback=None):
        ...
        if self._verbose:
            print("{} BG thread: Finished checking for update, doing callback".format(
                self._addon))
        if callback:
